document.addEventListener('DOMContentLoaded', () => {
        ...
        secoes.forEach(secao => {
            if ((mostrandoPendentes && secao.status !== 'concluida') || (!mostrandoPendentes && secao.status === 'concluida')) {
                ...
                secaoElement.innerHTML = `
                    ...
                    <button style="font-size: 1.1rem; color: green; font-weight: bold;"  onclick="adicionarTarefa(${secao.id})" id="btnAddTarefa">+</button>
                    <button onclick="excluirSecao(${secao.id})" id="btnRemoveTarefa">🗑️</button>
                    <label class="switch">
    ...
    window.adicionarTarefa = (secaoId) => {
        ...
        document.getElementById('dataVencimentoTarefa').valueAsDate = new Date();
